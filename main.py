...
NUMBERS_OF_TEACHERS = 5
NUMBERS_OF_STUDENTS = 60
...
def seed_teacher():
    teachers = [fake.name() for _ in range(NUMBERS_OF_TEACHERS)]
...
def seed_students():
    students = [fake.name() for _ in range(NUMBERS_OF_STUDENTS)]
    list_group_id = [randint(1, len(groups)) for _ in range(NUMBERS_OF_STUDENTS)]
    sql = "INSERT INTO students(fullname, group_id) VALUES (?, ?);"
    cursor.executemany(sql, zip(students, list_group_id))


def seed_subjects():
    list_teacher_id = [randint(1, NUMBERS_OF_TEACHERS) for _ in range(len(subjects))]
...
def seed_grades():
    ...
    for day in list_date:
        ...
        random_students = [randint(1, NUMBERS_OF_STUDENTS) for _ in range(7)]
